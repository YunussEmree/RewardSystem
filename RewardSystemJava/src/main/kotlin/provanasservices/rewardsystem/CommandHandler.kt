...
class CommandHandler(private val plugin: JavaPlugin) : CommandExecutor {
    override fun onCommand(sender: CommandSender, command: Command, label: String, args: Array<String>): Boolean {
        ...
        } else if (args[0].equals("mob", ignoreCase = true) && sender.hasPermission("rewardsystem.mobspawn")){
            var type = args[1]
            var hp = args[2]
            var damage = args[3]
            var name = (command.name.replace(args[0], "").replace(args[1], "").replace(args[2], "").replace(args[3], ""))
        }
        return true
    }
}