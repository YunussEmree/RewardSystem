...
class Events(private var plugin: Main) : Listener {
                    ...
                    reward.rewardMessages!!.forEach { message: String ->
                        Bukkit.getOnlinePlayers().forEach { onlinePlayer: Player ->
                            onlinePlayer.sendMessage(
                                translateColors(replacePlaceholders(message, player!!.name, selectedMap, plugin))
                            )
                        }
                    }
                } else {
    ...
    companion object {
        private fun replacePlaceholders(string: String, playerName: String, map: HashMap<String, Double>, plugin: Plugin): String {
            ...
            var noplayer = plugin.config.getString("NoPlayer");
            newStr = newStr.replace("%top_name_(\\d+)%".toRegex(), noplayer.toString())
        ...
        private fun chance(chance: Double) {
