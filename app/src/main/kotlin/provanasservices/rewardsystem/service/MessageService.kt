...
object MessageService {
    ...
    fun sendRewardMessages(
        ...
        if (reward.radius == -1) {
            sendMessageToAllPlayers(entity, reward, damageMap, finalDamager, noOne)
        } else {
    ...
    /**
     * Sends reward messages to players within a certain radius.
     */
    ...
    fun replacePlaceholders(
        ...
        for (match in namePattern.findAll(result)) {
            ...
            result = if (index < sortedEntries.size) {
                result.replace(match.value, sortedEntries[index].key)
            } else {
                result.replace(match.value, noOne)
            }
        }
        ...
        for (match in damagePattern.findAll(result)) {
            val index = match.groupValues[1].toInt() - 1
            result = if (index < sortedEntries.size) {
                result.replace(match.value, sortedEntries[index].value.toInt().toString())
            } else {
                result.replace(match.value, "0")
            }
        }
        ...
        result = result.replace(
            "%personal_damage%",
            (damageMap[playerName]?.toInt() ?: 0).toString()
        )
        result = result.replace("%final_damager%", finalDamager)
        result = result.replace("%killer%", finalDamager)
        result = result.replace("%entity%", entity.type.name)
        result = result.replace("%world%", entity.world.name)
        val loc = entity.location
        val pos = "${loc.blockX}, ${loc.blockY}, ${loc.blockZ}"
        result = result.replace("%position%", pos)
    ...
    fun sendCooldownMessage(
        ...
        player.sendMessage(
            ColorUtils.translateColors(
                message.replace("%time%", remainingTime.toString())
            )
        )
    }
